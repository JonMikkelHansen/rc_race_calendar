...
<gpx creator="Road Code Race Centre - https://www.roadcode.cc"
    ...
    gpxXML += `
    <trk>
        <src>Generated by lustport76_ using La Flamme Rouge track Editor</src>
        <link href="/maps/viewtrack/559045"><text>Strade Bianche ME 2024</text></link>
        <name>Strade Bianche ME 2024</name>
        <trkseg>`;
      
    trackpoints.forEach((trackpoint) => {
      gpxXML += `
            <trkpt lat="${trackpoint.latitude}" lon="${trackpoint.longitude}">
                <ele>${trackpoint.elevation || 0}</ele>
                <time>${trackpoint.time || new Date().toISOString()}</time>
            </trkpt>`;
    });
